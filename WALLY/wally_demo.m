...
methods  
    % constructor
    function this = wally_demo(L, shoulders_TORSO, s_na, s_range, move_prefs, CONFIG)
        if nargin > 0
            this.L = L;
...
%         scatter(this.TORSO_ABS(1),this.TORSO_ABS(2),'BLACK')
...
%         disp('good_holds')
    ...
    function [path, iters] = calc_path(this, wall, start_node)
...
%             scatter(path{i+1,2}(1),path{i+1,2}(2),'GREEN');
    ...
    function chunked_path = chunk_path_coms(this, path)
        ...
        SPACING_TORSO = 4; %number of moves between torso stances (between start to center, then again for center to final
        SPACING_ARM = 2; %number of moves between arm stances (not counting transition step)
        chunked_path = [];
...
%             scatter(path{i+1,2}(1),path{i+1,2}(2),'GREEN');
    ...
    function chk_path_coms = prep_coms(~, chk_path_coms)
        %Convert WALLY angle convetion to servo angle convention, in
        %degrees
    ...
    function [cur_fig, handles] = plot_ws_body(this, show, varargin)
       ...
       elseif show == 4
        ...
        if show == 3
            ...
            y = ws(:,2);
            bound = boundary(x, y);
            handles(end+1) = fill(x(bound),y(bound),'YELLOW');
            hold off  
        end
